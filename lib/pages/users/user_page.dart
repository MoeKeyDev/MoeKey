...
import 'package:moekey/pages/users/user_files.dart';
...
class UserPage extends HookConsumerWidget {
  const UserPage({super.key, this.username, this.host, this.userId});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    var userProvider = userInfoProvider(
      username: username,
      host: host,
      userId: this.userId,
    );
        ...
        body: GestureDetector(
          ...
          child: Center(
            child: Column(
              ...
              children: [
                Text(user.error.toString(), textAlign: TextAlign.center),
                Text(user.stackTrace.toString(), textAlign: TextAlign.center),
          ...
          items: [
            MkTabBarItem(
              label: Tab(
                child: Row(
                  children: [
                    const Icon(TablerIcons.home, size: 14),
                    Text(
                      S.current.overviews,
              ...
              child: UserOverview(
                userId: userId,
                onShowMoreFiles: () => selectedTab.value = 2,
              ),
            ),
            MkTabBarItem(
              label: Tab(
            ...
            MkTabBarItem(
              label: Tab(
                child: Row(
                  children: [
                    const Icon(TablerIcons.photo, size: 14),
                    Text(S.current.files, style: const TextStyle(fontSize: 12)),
                  ],
                ),
              ),
            ...
            if (userData.publicReactions || userData.id == loginUser?.id)
              MkTabBarItem(
                label: Tab(
                  child: Row(
                    children: [
                      const Icon(TablerIcons.mood_happy, size: 14),
                      Text(
                        S.current.reaction,
                        style: const TextStyle(fontSize: 12),
                      ),
            ...
            MkTabBarItem(
              label: Tab(
                child: Row(
                  children: [
                    const Icon(TablerIcons.paperclip, size: 14),
